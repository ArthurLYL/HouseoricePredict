...
# 将顺序变量（ordinal variable），例如（BsmtQual：Ex，Gd，TA，Fa，Po），转成离散数字表示，LabelEncoder不能很好的表示值之间的顺序关系，因此使用自定义函数
def ord_to_int(x):
    if x == 'Ex':
        x = 0
    elif x == 'Gd':
...
names = ['1.ridge', '2.lasso', '3.svr', '4.ela_net', '5.bay_ridge', '6.ker_ridge', '7.neural network']
...
class grid:
    ...
    def random_grid_get(self, X, y, param_grid):
...
param_dist = {'alpha': np.linspace(0.0001, 0.002, 20),
...
if auto_grid:
    ...
    grid(MLPRegressor()).random_grid_get(X_scaled, Y_log, param_dist)
...
model_neuralnetwork = MLPRegressor(alpha=0.0017000000000000001, learning_rate='adaptive', solver='adam')
"""STEP7:集成多个模型-------------------------------------------------------------------------------------"""
...
weight_avg1 = AverageWeight(
    mod=[model_lasso, model_ridge, model_svr, model_kernelridge, model_elasticnet, model_bayesianridge, model_neuralnetwork],
    weight=[0.02, 0.2, 0.25, 0.3, 0.02, 0.2, 0.01])
...
weight_avg2 = AverageWeight(
    mod=[model_lasso, model_elasticnet],
    weight=[0.5, 0.5]
)
